...
Процедура ЗаполнитьТекущиеНастройки()
	...
	ОписаниеНастроек = КонтрольРаботыПользователейСлужебный.НастройкиРегистрацииСобытийДоступаКДанным();
	НастройкиСобытия = ОписаниеНастроек.Настройки;
	ОбщийКомментарий = ОписаниеНастроек.ОбщийКомментарий;
	
	Комментарии = Новый Соответствие;
	Для Каждого КлючИЗначение Из ОписаниеНастроек.Комментарии Цикл
...
&НаСервере
Функция КартинкаВКонфигураторе(ОбъектМетаданных, Представление, ИмяКоллекции = "НеУказана", ПоляКоллекции = Ложь)
...
Функция ЗаписаноНаСервере()
	...
	КонтрольРаботыПользователейСлужебный.УстановитьНастройкиРегистрацииСобытийДоступаКДанным(НастройкиСобытия,
		Комментарии, ОбщийКомментарий);
