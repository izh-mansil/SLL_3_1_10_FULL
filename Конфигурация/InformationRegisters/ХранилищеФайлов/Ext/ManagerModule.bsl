...
Процедура ПеренестиДанные(СообщатьПрогресс = Ложь) Экспорт
	...
	Если СообщатьПрогресс Тогда
		...
		Шаблон = НСтр("ru = 'Обработано %1 (%2 Мб) файлов из %3 (%4 Мб)'");
		Текст = СтрШаблон(Шаблон, 0, 0, ВсегоЗаписей, ВсегоРазмерМб);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СообщатьПрогресс Тогда
			...
			Если ЗаписейОбработано % 100 = 0 Тогда
				...
				Текст = СтрШаблон(Шаблон, ЗаписейОбработано, Окр(РазмерОбработано / 1024 / 1024), ВсегоЗаписей, ВсегоРазмерМб);
