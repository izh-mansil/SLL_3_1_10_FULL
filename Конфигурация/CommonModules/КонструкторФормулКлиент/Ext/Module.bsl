...
Процедура НачатьПоискВСпискеПолей(Форма) Экспорт
	...
	Если ДлинаСтрокиОтбора >= ПодключенныйСписокПолей.КоличествоСимволовДляНачалаПоиска Тогда
		...
		Если СтрокаРезультатовПоиска.Заголовок <> "" И СтрНачинаетсяС(Отбор, СтрокаРезультатовПоиска.Заголовок) Тогда
		...
		Если Результат.Свойство("НайденныеЭлементы") Тогда
			...
			Для Каждого НайденныйЭлемент Из НайденныеЭлементы Цикл
				...
				Если РодительЭлемента <> Неопределено Тогда
					...
					РодительЭлемента.Вес = Макс(РодительЭлемента.Вес, НайденныйЭлемент.Вес);
				КонецЕсли;
			КонецЦикла;
...
Процедура ОбработатьРезультатПоиска(Форма, АдресРезультата, ИдентификаторЗадания)
	...
	Для Каждого НайденныйЭлемент Из НайденныеЭлементы Цикл
		...
		Если РодительЭлемента <> Неопределено Тогда
			РодительЭлемента.ПодчиненныйЭлементСоответствуетОтбору = Истина;
			РодительЭлемента.Вес = Макс(РодительЭлемента.Вес, ДобавляемыйЭлемент.Вес);
		КонецЕсли;
	КонецЦикла;
	КонструкторФормулКлиентСервер.СортироватьПоКолонке(СтрокаРезультатовПоиска, "Вес");
	УстановитьТекущуюСтрокуНаПервуюНайденную(Форма.Элементы[ИмяСписка], ДеревоПолей);
...
Процедура УстановитьПредварительныйОтбор(Список, Отбор, СтрокаРезультатовПоиска, ИскатьВложенныеПоля = Неопределено)
	...
	Если ИскатьВложенныеПоля = Неопределено Тогда
	...
	Для Каждого Элемент Из Список.ПолучитьЭлементы() Цикл
		Если ЗначениеЗаполнено(Элемент.ПредставлениеПутиКДанным) Тогда
			...
			Если РезультатПоиска.СоответствуетОтбору И Не ИскатьВложенныеПоля Тогда
				...
				Иначе
					НовыйЭлемент = СтрокаРезультатовПоиска.ПолучитьЭлементы().Добавить();
			...
			Если Элемент <> СтрокаРезультатовПоиска И РезультатПоиска.СоответствуетОтбору Тогда
				Если ИскатьВложенныеПоля Тогда
					УстановитьПредварительныйОтбор(Элемент, Сред(Отбор, СтрДлина(СтрокиОтбора[0])+2), СтрокаРезультатовПоиска, Истина);
				Иначе
