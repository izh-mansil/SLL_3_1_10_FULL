...
Процедура ПриОпределенииНастроекРегистрацииСобытийДоступаКДанным(Настройки) Экспорт
	...
	Для Каждого Настройка Из ХранимыеНастройки.Настройки Цикл
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	ПользователиСлужебный.УдалитьНесуществующиеПоляИзНастройкиСобытияДоступДоступ(
		ХранимыеНастройки.Настройки, НенайденныеПоля);
...
//  Булево
//
Функция ВключенаРегистрацияДоступаКДанным() Экспорт
...
// Возвращаемое значение:
//  Структура:
//    * Настройки        - см. ХранимыеНастройкиРегистрации.Настройки
//    * Комментарии      - см. ХранимыеНастройкиРегистрации.Комментарии
//    * ОбщийКомментарий - см. ХранимыеНастройкиРегистрации.ОбщийКомментарий
//   
...
//  ОбщийКомментарий - см. ХранимыеНастройкиРегистрации.ОбщийКомментарий
//
Процедура УстановитьНастройкиРегистрацииСобытийДоступаКДанным(Настройки, Комментарии, ОбщийКомментарий) Экспорт
	...
	Попытка
		Блокировка.Заблокировать();
		ТекущиеНастройки = ХранимыеНастройкиРегистрации();
		ТекущиеНастройки.Настройки = Настройки;
		ТекущиеНастройки.Комментарии = Комментарии;
		ТекущиеНастройки.ОбщийКомментарий = ОбщийКомментарий;
		Хранилище = Новый ХранилищеЗначения(ТекущиеНастройки);
		Константы.НастройкиРегистрацииСобытийДоступаКДанным.Установить(Хранилище);
		Пользователи.ОбновитьНастройкиРегистрацииСобытийДоступаКДанным();
		ЗафиксироватьТранзакцию();
...
//        * Ключ     - Строка - полное имя поля, например "Справочник.ФизическиеЛица.Реквизит.НомерДокумента".
	...
	Для Каждого КлючИЗначение Из Результат Цикл
		Если Не Значение.Свойство(КлючИЗначение.Ключ)
		 Или ТипЗнч(Значение[КлючИЗначение.Ключ]) <> ТипЗнч(КлючИЗначение.Значение) Тогда
			Продолжить;
		КонецЕсли;
		Если КлючИЗначение.Ключ = "Настройки" Тогда
			Для Каждого Настройка Из Значение.Настройки Цикл
				Если ТипЗнч(Настройка) = Тип("ОписаниеИспользованияСобытияДоступЖурналаРегистрации") Тогда
					Результат.Настройки.Добавить(Настройка);
				КонецЕсли;
			КонецЦикла;
		Иначе
