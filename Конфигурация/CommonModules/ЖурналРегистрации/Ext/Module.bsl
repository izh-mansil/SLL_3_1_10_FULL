...
Процедура ПрочитатьСобытияЖурналаРегистрации(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	Для Каждого СобытиеЖурнала Из СобытияЖурнала Цикл
		...
		СобытиеЖурнала.Комментарий = СтрокаБезНедопустимыхСимволов(СобытиеЖурнала.Комментарий);
...
Функция ПредставлениеЗначения(Ключ, Значение)
	...
	Если ТипЗнч(Значение) <> Тип("Массив") Тогда
		Если ВРег(Ключ) = ВРег("ИмяСобытия") И СтрНачинаетсяС(Значение, "_$") Тогда
			...
			КонецЕсли;
		КонецЕсли;
		Возврат СтрокаБезНедопустимыхСимволов(Формат(Значение, ФорматнаяСтрока));
	КонецЕсли;
	...
	Иначе
		...
		Для Каждого ТекущееЗначение Из Значение Цикл
			Строки.Добавить(СтрокаБезНедопустимыхСимволов(Формат(ТекущееЗначение, ФорматнаяСтрока)));
		КонецЦикла;
		Соединитель = ", ";
	КонецЕсли;
...
Функция СтрокаБезНедопустимыхСимволов(Знач ИсходнаяСтрока)
	
	Пока Истина Цикл
		Позиция = ОбработкаСтрокиXML.НайтиНедопустимыеСимволыXML(ИсходнаяСтрока);
...
Процедура УстановитьСтрокуДанных(СобытиеЖурнала)
	...
	УточненнаяСтрока = СтрокаБезНедопустимыхСимволов(ДанныеСтрокой);
