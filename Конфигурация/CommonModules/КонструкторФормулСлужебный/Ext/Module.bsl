...
Функция ЗапуститьПоискВСпискеПолей(СтруктураФормы) Экспорт
	...
	Если ОтборУстановлен Тогда
		...
		СтруктураФормы.Вставить("КешДляСКД", ТаблицаКеша);
...
Процедура ДобавитьСписокПолейНаФорму(Форма, Параметры) Экспорт
	...
	Если АдресОписанияДлительнойОперации = Неопределено Тогда
		...
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("КешДляСКД", Новый ОписаниеТипов("ТаблицаЗначений")));
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ИмяСпискаПолей", Новый ОписаниеТипов("Строка"), "КешДляСКД"));
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ТипПоля", Новый ОписаниеТипов("ОписаниеТипов"), "КешДляСКД"));
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ДоступныеПоля", Новый ОписаниеТипов("ТаблицаЗначений"), "КешДляСКД"));
		Для Каждого ОписаниеРеквизита Из РеквизитыПодключаемогоСписка() Цикл
			ИмяРеквизита = ОписаниеРеквизита.Ключ;
			ТипРеквизита = ОписаниеРеквизита.Значение;
			ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы(ИмяРеквизита, ТипРеквизита, "КешДляСКД.ДоступныеПоля"));
		КонецЦикла;
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ЕстьПодчиненные", Новый ОписаниеТипов("Булево"), "КешДляСКД.ДоступныеПоля"));
	КонецЕсли;
...
Процедура ЗаполнитьСписокДоступныхРеквизитов(Параметры, ИсточникиДоступныхПолей, КешСКД = Неопределено)
	...
	Если КешСКД <> Неопределено И ТипУказан Тогда
		ОтборКеша = Новый Структура("ИмяСпискаПолей, ТипПоля", ИмяСпискаПолей, ТекущийРеквизит.Тип);
		ДанныеКеша = КешСКД.НайтиСтроки(ОтборКеша);
	...
	Если ДоступныеРеквизиты = Неопределено Тогда
		...
		Если КешСКД <> Неопределено И ТипУказан И ДанныеКеша.Количество() = 0 Тогда
			ДанныеКеша = КешСКД.Добавить();
...
Процедура РазвернутьРеквизит(ИдентификаторСтроки, ИмяСписка, Форма)
	...
	КешДляСКД = ?(ТипЗнч(Форма) = Тип("Структура"), Форма.КешДляСКД, Форма.РеквизитФормыВЗначение("КешДляСКД"));
	...
	ПараметрыЗаполненияСпискаДоступныхРеквизитов.НастройкиСписка = НастройкиСписка;
	ЗаполнитьСписокДоступныхРеквизитов(ПараметрыЗаполненияСпискаДоступныхРеквизитов, ИсточникиДоступныхПолей, КешДляСКД);
	
	Если ТипЗнч(Форма) = Тип("Структура") Тогда
		Форма.КешДляСКД = КешДляСКД;
	Иначе
		Форма.ЗначениеВРеквизитФормы(КешДляСКД, "КешДляСКД");
	КонецЕсли;
