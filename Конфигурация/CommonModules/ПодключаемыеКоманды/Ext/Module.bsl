...
Процедура ВывестиКоманды(Форма, Команды, ПараметрыРазмещения)
	...
	Для Каждого СведенияОПодменюКоманды Из СведенияОВсехПодменю Цикл
		Для Каждого ИндексКоманды Из СведенияОПодменюКоманды.Команды Цикл
			...
			ИмяКорневогоЭлемента = ?(Команда.Вид = "КоманднаяПанель", Команда.ИмяВФорме, Подменю.Имя);
			...
			Если СведенияОПодменюКоманды.КоличествоКоманд = 1 И ГруппаФормы.Родитель <> Форма 
				И Команда.Вид <> "КоманднаяПанель" Тогда
