...
Функция РазложитьПредставлениеАдресаПоПолям(Представление, ТекущийАдрес = Неопределено) Экспорт
	...
	Для Каждого ЧастьАдрес Из НераспределенныеДанные Цикл
		...
		Если ПустаяСтрока(ЧастьАдрес.ТипОбъекта) И Не ЗаполненУровеньУлицы Тогда
			Если ЗаполнитьПоляГорода(Адрес, ЧастьАдрес) Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
...
Функция ЗаполнитьПоляГорода(Адрес, ЧастьАдрес)
	
	ИменаУровнейАдресаПоКоду = ИменаУровнейАдресаПоКоду();
	Для Каждого Уровень Из ИменаУровнейАдресаПоКоду Цикл
		Если Уровень.Ключ = 3 Или Уровень.Ключ = 8 Тогда
			Продолжить;
		КонецЕсли;
		Если Адрес.Свойство(Уровень.Значение) И ПустаяСтрока(Адрес[Уровень.Значение]) Тогда
			Адрес[Уровень.Значение] = ЧастьАдрес.Значение;
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
