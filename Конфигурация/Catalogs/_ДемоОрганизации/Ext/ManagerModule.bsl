	...
	Если ТипЗнч(ДополнительныеПараметры.ВладелецШаблона) = Тип("СправочникСсылка._ДемоПодразделения") Тогда
		...
		Если Проект <> Неопределено Тогда
			ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
				Сообщение.ДополнительныеПараметры.ПараметрыСообщения.Проект, 
		...
		Если ДополнительныеПараметры.ПараметрыСообщения.Свойство("РегламентПроекта") Тогда
			ДанныеФайлы = РаботаСФайлами.ДанныеФайла(ДополнительныеПараметры.ПараметрыСообщения.РегламентПроекта,
				ДополнительныеПараметры.ПараметрыСообщения.УникальныйИдентификатор);
...
Функция ПодписиИПечатиОрганизации(Организация) Экспорт
	...
	ЧтениеОрганизацииРазрешено = ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Организация, "Ссылка",
		Истина));
	...
	Для Каждого ПодписьПечать Из ПодписиИПечати Цикл
		...
		ФайлыКартинок.Добавить(ФайлКартинки);
	КонецЦикла;
	ДвоичныеДанныеКартинок = РаботаСФайлами.ДвоичныеДанныеФайлов(ФайлыКартинок, Ложь);
	Для Каждого ПодписьПечать Из ПодписиИПечати Цикл
		Идентификатор = ПодписьПечать.Ключ;
		ФайлКартинки = ПодписьПечать.Значение;
		Картинка = КартинкаИзФайла(ДвоичныеДанныеКартинок, ФайлКартинки);
...
Функция КартинкаИзФайла(ДвоичныеДанныеКартинок, Файл)

	ДвоичныеДанные = Неопределено;

	Если ЗначениеЗаполнено(Файл) Тогда
		ДвоичныеДанные = ДвоичныеДанныеКартинок[Файл];
	КонецЕсли;
...
Функция ДанныеПечати(ИсточникиДанных, КодЯзыка, ДополнительныеПараметры)
	...
	ДанныеПечати.Колонки.Добавить("ПечатьОрганизации",
		Новый ОписаниеТипов("СправочникСсылка._ДемоОрганизацииПрисоединенныеФайлы"));
	ДанныеПечати.Колонки.Добавить("ПодписьДиректора",
		Новый ОписаниеТипов("СправочникСсылка._ДемоОрганизацииПрисоединенныеФайлы"));
	ДанныеПечати.Колонки.Добавить("ПодписьГлавногоБухгалтера",
		Новый ОписаниеТипов("СправочникСсылка._ДемоОрганизацииПрисоединенныеФайлы"));
	...
	ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(ИсточникиПоТипам[Тип("СправочникСсылка._ДемоОрганизации")], 
		"ПечатьОрганизации, ПодписьДиректора, ПодписьГлавногоБухгалтера");
	...
	Для Каждого ОписаниеИсточникаДанных Из ДополнительныеПараметры.ОписанияИсточниковДанных Цикл
		...
		Если ЗначениеЗаполнено(Владелец) И ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Владелец)) И ОбщегоНазначения.ЭтоДокумент(
			Владелец.Метаданные()) Тогда
			Владельцы.Добавить(Владелец);
		КонецЕсли;
		
		Если ТипЗнч(ОписаниеИсточникаДанных.Значение) = Тип("СправочникСсылка._ДемоОрганизации") Тогда
			Организации.Добавить(ОписаниеИсточникаДанных.Значение);
		КонецЕсли;
	КонецЦикла;
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Организации") Тогда
		...
		|ГДЕ
		|	Справочник._ДемоОрганизации.Ссылка В (&Организации)";
	...
	Для Каждого ОписаниеИсточникаДанных Из ДополнительныеПараметры.ОписанияИсточниковДанных Цикл
		...
		Иначе
			Строка = ТаблицаОрганизаций.Найти(Объект, "Ссылка");
