		...
		Если НедопустимыйПользователь <> Неопределено Тогда
			...
			Если ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
				Если НедопустимыйПользователь.Служебный Тогда
					ТекстДляОднойОшибки      = НСтр("ru = 'Права (роли) служебного пользователя ""%2"" настраиваются только программно.'");
					ТекстДляНесколькихОшибок = НСтр("ru = 'Права (роли) служебного пользователя ""%2"" в строке %1 настраиваются только программно.'");
				Иначе
