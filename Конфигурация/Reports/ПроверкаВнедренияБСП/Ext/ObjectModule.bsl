...
Перем ДоступноИспользованиеРегулярныхВыражений; // Булево
...
Функция МинимальнаяВерсияДляИспользованияРегулярныхВыражений()
...
Процедура Подключаемый_КонтактнаяИнформация_ПроверитьВнедрение()
	...
	Для Каждого ПроверяемыйОбъект Из ТаблицаВидов Цикл
		...
		Если МодульФормы.Свойство("Структура") Тогда
...
Функция ПроверитьПраваНаРеквизитыВРоли(ДокументDOM, ИмяРоли, Исправлять)
	...
	Для Каждого РеквизитВРоли Из ТаблицаРеквизитовВРоли Цикл
		...
		Если ПозицияСтандартнойТабличнойЧасти > 0 Тогда
			ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(Сред(РеквизитВРоли.ПолноеИмяОбъекта, 1, ПозицияСтандартнойТабличнойЧасти - 2));
		...
		ИначеЕсли ПозицияСтандартныхРеквизитов > 0 Тогда
			ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(Сред(РеквизитВРоли.ПолноеИмяОбъекта, 1, ПозицияСтандартныхРеквизитов - 2));
			ПутьДоРеквизита = Сред(РеквизитВРоли.ПолноеИмяОбъекта, ПозицияСтандартныхРеквизитов);
			ПреобразоватьПутьДоСтандартныхРеквизитов(ПутьДоРеквизита, СинонимыСтандартныхРеквизитов);
			ИмяРеквизита = ОбъектМетаданных.ПолноеИмя() + ПутьДоРеквизита;
		Иначе
			ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(РеквизитВРоли.ПолноеИмяОбъекта);
			Если ОбъектМетаданных = Неопределено Тогда
...
Процедура ПроверитьИспользованиеПроксиВФайле(ПараметрыОшибкиИспользованияПрокси)
	...
	Пока Истина Цикл
		...
		Если ДоступноИспользованиеРегулярныхВыражений() Тогда
			ВыражениеДляПоиска = СтрСоединить(МассивМетодовСПараметромПрокси, "|");
			ВыражениеДляПоиска = СтрЗаменить(ВыражениеДляПоиска, "(", "\(");
			
			РезультатПоиска = ВычислитьСтрНайтиПоРегулярномуВыражению(ТекстМодуля, ВыражениеДляПоиска, ПозицияМетода + 1);
		...
		Иначе
			РезультатПоискаМетода = СтрНайтиПоМассиву(ТекстМодуля, МассивМетодовСПараметромПрокси,, ПозицияМетода + 1);
			Если НЕ РезультатПоискаМетода.Успешно Тогда
				Прервать;
			КонецЕсли;
			
			НайденныйМетод = РезультатПоискаМетода.НайденныйЭлемент;
			ПозицияМетода = РезультатПоискаМетода.Позиция;
		КонецЕсли;
...
Процедура ПроверитьОтложенныеОбработчикиНаНаличиеНеразделенныхИзменяемыхОбъектов()
	...
	Для Каждого ОбработчикОбновления Из ОбработчикиОбновленияОтложенно Цикл
		...
		Если СтрСравнить(ПроцедураОбработчикаОбновления, "РегистрыСведений.НастройкиТранспортаОбменаДанными.ОбработатьДанныеДляПереходаНаНовуюВерсию") = 0
			Или СтрСравнить(ПроцедураОбработчикаОбновления, "ЗащитаПерсональныхДанных.ПеренестиЗащищаемыеОбластиПерсональныхДанных") = 0 Тогда
			
			Продолжить; // Исключение из проверки.
		КонецЕсли;
...
Процедура УправлениеДоступом_ПроверитьВставкуТекстаВМодулеФормы(Параметры, ОбъектМетаданныхФорма)
	...
	Если Модуль.Свойство("Структура") Тогда
...
Функция ВхожденияСтроки(ТекстМодуля, СтрокаПоиска, ПроверитьЧтоСледующийСимволИмениПеременной = Ложь)
	...
	Если ДоступноИспользованиеРегулярныхВыражений() Тогда
		...
		Пока Истина Цикл
			РезультатПоиска = ВычислитьСтрНайтиПоРегулярномуВыражению(ТекстМодуля, ВыражениеДляПоиска, Позиция + 1);
			Если РезультатПоиска = Неопределено Тогда
				Прервать;
			КонецЕсли;
...
Функция ДоступноИспользованиеРегулярныхВыражений()
	
	Если ДоступноИспользованиеРегулярныхВыражений <> Неопределено Тогда
		Возврат ДоступноИспользованиеРегулярныхВыражений;
	КонецЕсли;
	
	МинимальнаяТребуемаяВерсияПлатформы = МинимальнаяВерсияДляИспользованияРегулярныхВыражений();
	
	СистемнаяИнформация = Новый СистемнаяИнформация;
	ВерсияПлатформы = СистемнаяИнформация.ВерсияПриложения;
	
	РезультатСравненияВерсий = ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияПлатформы,
		МинимальнаяТребуемаяВерсияПлатформы);
	
	ДоступноИспользованиеРегулярныхВыражений = (РезультатСравненияВерсий >= 0);
	
	Возврат ДоступноИспользованиеРегулярныхВыражений;
	
КонецФункции

Функция ВычислитьСтрНайтиПоРегулярномуВыражению(Текст, ВыражениеДляПоиска, НачальнаяПозиция = 1)
	
	Выражение = "СтрНайтиПоРегулярномуВыражению(Текст, ВыражениеДляПоиска,, НачальнаяПозиция)";
...
Функция НайтиПроцедуруМодуля(Модуль, ИмяПроцедуры)
	...
	Если Не ДоступноИспользованиеРегулярныхВыражений() Тогда
		// Обратная совместимость на случай, если работы с регулярными выражениями нет в текущей версии платформы.
	...
	РезультатПоискаНачалоПроцедуры = ВычислитьСтрНайтиПоРегулярномуВыражению(ТекстМодуля, ВыражениеДляПоиска);
	...
	// Поиск первого слова "КонецПроцедуры" после объявления найденной процедуры.
	// Также в начале строки.
	ВыражениеДляПоиска = "(?m)^КонецПроцедуры";
	РезультатПоискаКонецПроцедуры = ВычислитьСтрНайтиПоРегулярномуВыражению(ТекстМодуля, ВыражениеДляПоиска, НачальнаяПозиция);
...
Процедура ДобавитьПараметрМетодаМногопоточнойОперации(ПараметрыМетода, ШаблонИмениПроцедуры, ИмяПодсистемы,
	...
	УправлениеДоступом_ШаблонИмениПроцедуры = "Подключаемый_УправлениеДоступом_%1(ПроверяемыеОбъекты)";
	
	Для Каждого Проверка Из ПроверкиПодсистемыУправлениеДоступом Цикл
		ИмяПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(УправлениеДоступом_ШаблонИмениПроцедуры, Проверка);
		
		МассивПараметров = Новый Массив;
		МассивПараметров.Добавить(ИмяПроцедуры);
		МассивПараметров.Добавить(ИмяПодсистемы);
		МассивПараметров.Добавить(ХранилищеСтруктурыИнициализации);
		МассивПараметров.Добавить(Проверка);
...
Функция УправлениеДоступом_МассивПроверок()
	...
	Проверки.Добавить("ПроверитьВставкуТекстаВМодулеФормы");
	Проверки.Добавить("ПроверитьИспользованиеПредопределенныхВМодулеФормы");
	Проверки.Добавить("ПроверитьИспользованиеПредопределенныхВОбщемМодуле");
	Проверки.Добавить("ПроверитьИспользованиеПредопределенныхВМодуляхОбъекта");
	Проверки.Добавить("ПроверитьИспользованиеПредопределенныхВМодулеКоманды");
	Проверки.Добавить("ПроверитьВставкуПроцедурыПриЗаполненииОграниченияДоступаВМодулеМенеджераОбъекта");
