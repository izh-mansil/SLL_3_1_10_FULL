...
Процедура ЗаполнитьИсключения()
	...
	Пока ИмяОбласти <> "КонецДокумента" Цикл
		Область = ТабличныйДокумент.Область(НомерСтроки, 0);
		ИмяОбласти = Область.Имя;
		ТекстСтроки = Область.Текст;
		
		Если Не ПримерЗакончен Тогда
			ПримерЗакончен = (ИмяОбласти = "СписокИсключений");
			НомерСтроки = НомерСтроки + 1;
			Продолжить;
		КонецЕсли;
		
		Если ПустаяСтрока(ТекстСтроки) Или ИмяОбласти = "КонецДокумента" Тогда
			НомерСтроки = НомерСтроки + 1;
			Продолжить;
		КонецЕсли;
		
		Если ИмяОбласти = "ИсключаемыеОбъекты" Тогда
			ТекущийБлокИсключений = "ИсключаемыеОбъекты";
			НомерСтроки = НомерСтроки + 1;
			Продолжить;
		ИначеЕсли ИмяОбласти = "ИсключаемыеСвязи" Тогда
...
Процедура НайтиОбращениеКМетаданнымВКодеМодулей(СтруктураПоискаВызова)
	...
	Для Каждого СтрокаПоиска Из МассивСтрокПоиска Цикл
		...
		Пока Истина Цикл
			...
			ТекстВызова = Лев(НоваяСтрокаТекста, КонецВызова);
			СледующийСимвол = Сред(НоваяСтрокаТекста, КонецВызова + 1, 1);
			Проверка = Неопределено;
			// АПК:280-выкл проверка при помощи конструктора структуры, исключение является корректным поведением.
			Попытка
				Проверка = Новый Структура(СледующийСимвол);
			Исключение
				// Обработка исключения не требуется.
				// Если ошибка - значит найден корректный вызов.
			КонецПопытки;
			// АПК:280-вкл.
			
			Если (Проверка = Неопределено Или Проверка.Количество() = 0)
				И Не ЭтоКомментарий(СтрокаТекста, НачалоВызова) Тогда
...
Процедура ЗаполнитьПроверяемыеНаАвтономностьОбъекты()
	...
	ТабличныйДокумент = Отчеты.ЗависимостиПодсистем.ПолучитьМакет("АвтономныеОбработки");
	...
	Пока ИмяОбласти <> "КонецДокумента" Цикл
		...
		ИмяОбласти = Область.Имя;
		ТекстСтроки = Область.Текст;
		Если ПустаяСтрока(ТекстСтроки)
			Или ИмяОбласти = "Шапка"
			Или ИмяОбласти = "КонецДокумента" Тогда
			НомерСтроки = НомерСтроки + 1;
			Продолжить;
		КонецЕсли;
