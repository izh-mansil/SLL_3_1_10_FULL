...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	БезУдаленных = Истина;
	ИмяКлючевогоРеквизита = ИмяКлючевогоРеквизитаПоУмолчанию();
...
Процедура Подключаемый_ТабличныйДокументНачалоВводаЗначения(Элемент, Область, СтандартнаяОбработка)
	...
	Если Область.СодержитЗначение Или Область.Верх = 1 Тогда
		Возврат;
	КонецЕсли;
...
Функция КолонкаТабличногоДокументаПоИмени(ИмяТабличногоДокумента, ИмяКолонки = "")
	...
	Для НомерКолонки = 1 По ТабличныйДокумент.ШиринаТаблицы Цикл
		...
		Результат = ЭтотОбъект[ИмяТабличногоДокумента].Область(1, НомерКолонки);
		
		КоординатыОбласти = Новый Структура;
		КоординатыОбласти.Вставить("Верх", 1);
		КоординатыОбласти.Вставить("Лево", ТекущиеИмяПредопределенныхДанныхКолонка);
		Если Результат.Имя = ИмяКолонки Или (Результат.Имя = ИмяОбласти(КоординатыОбласти) И Результат.Текст = ИмяКолонки) Тогда
...
Процедура ТабличныйДокументВыборЗавершениеВыбораРеквизитаСсылочногоТипа(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не ЭтоПеречисление(ДополнительныеПараметры.ТипЗначения) 
		И ТипМожетСодержатьПредопределенныеЗначения(ДополнительныеПараметры.ТипЗначения) 
		И ЗначениеРеквизитаОбъекта(Результат, "Предопределенный") Тогда
...
Процедура ТабличныйДокументПриИзмененииОбласти(ИмяТабличногоДокумента, ОбластьЯчеек)
	...
	Для НомерКолонки = ОбластьИзмененияЛево По ОбластьИзмененияПраво Цикл
		...
		Если ИмяТабличнойЧасти = "" Тогда
			Если (Не Локализуемый И Не ПолнаяСтруктураОбъекта.Свойство(ИмяРеквизита, РеквизитСтруктуры))
				Или (Локализуемый И Не ПолнаяСтруктураОбъекта.Свойство(ОсновноеИмяРеквизитаЛ, РеквизитСтруктуры)) Тогда
				Продолжить;
			КонецЕсли;
		ИначеЕсли ПолнаяСтруктураОбъекта.Свойство(ИмяТабличнойЧасти, РеквизитСтруктуры) Тогда
			РеквизитСтруктурыТЧ = Неопределено;
			Если (Не Локализуемый И Не РеквизитСтруктуры.Реквизиты.Свойство(ИмяРеквизита, РеквизитСтруктурыТЧ))
				Или (Локализуемый И Не РеквизитСтруктуры.Реквизиты.Свойство(ОсновноеИмяРеквизитаЛ, РеквизитСтруктурыТЧ)) Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ТабличныйДокументПриИзмененииТипизацияЗначений(ИмяТабличнойЧасти)
	...
	Для каждого СтрокаТаблицы Из ТаблицаДанныхОбласти Цикл
		...
		ИначеЕсли ЭтоСсылочныйТип(ТипРеквизитаСтруктуры) Тогда
			...
			//@skip-check query-in-loop - разовые вызовы при очистке ячеек, при этом типы данных могут быть произвольные.
...
Процедура ОбъектМетаданныхПриИзмененииНаСервере()
	...
	УстановитьИмяКлючевогоРеквизита(МетаданныеОбъекта);
...
&НаСервере
Процедура УстановитьИмяКлючевогоРеквизита(МетаданныеОбъекта)
	...
	Попытка
		...
		ИмяКлючевогоРеквизита = ПользовательскиеНастройкиЗаполненияЭлементов.ИмяКлючевогоРеквизита;
		
		Если ПустаяСтрока(ИмяКлючевогоРеквизита) Тогда
			ИмяКлючевогоРеквизита = ИмяКлючевогоРеквизитаПоУмолчанию();
		КонецЕсли;
...
Процедура ПреобразоватьТабличныеДанныеВКод(ИсточникДанных, ДанныеИнициализации = Неопределено, ИмяТабличнойЧасти = "")
	...
	Для каждого СтрокаДанных Из ИсточникДанных Цикл
		...
		Если ЕстьКлючеваяКолонка И ПустаяСтрока(ИмяТабличнойЧасти) И ВыводитьИменПредопределенныхДанных Тогда
			...
			Если Не ЗначениеЗаполнено(ИмяПредопределенныхДанных) 
				Или УникальныеИменаСтрок.Получить(ИмяПредопределенныхДанных) = Истина Тогда
		...
		Для каждого Реквизит Из РеквизитыИнициализации Цикл
			...
			Иначе//Если ЭтоСсылочныйРеквизит Тогда
				...
				ИначеЕсли Метаданные.Справочники.Найти(МетаданныеРеквизита.Имя) <> Неопределено
					...
					Предопределенный = (СтрСравнить(ИмяКлючевогоРеквизита, "ИмяПредопределенныхДанных") = 0 И ЗначениеРеквизита.Предопределенный)
						Или (ЕстьКлючеваяКолонка И ЗначениеЗаполнено(СтрокаДанных[ИмяКлючевогоРеквизита]));
					
					Если Предопределенный И ЗначениеЗаполнено(ЗначениеРеквизита.ИмяПредопределенныхДанных) Тогда
						МенеджерОбъекта = "";
...
&НаСервереБезКонтекста
Функция ИмяКлючевогоРеквизитаПоУмолчанию()
