...
&НаКлиенте
Перем ОбъектыИсключенияПроверкиУсловныхВызовов;
&НаКлиенте
Перем ПодсистемыИсключенияПроверкиУсловныхВызовов;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДобавитьПроверку(НСтр("ru = 'Недопустимо наличие недокументированных связей между подсистемами'"), 
...
#Если ВебКлиент Или МобильныйКлиент Тогда
	ВызватьИсключение НСтр("ru = 'Для открытия обработки необходимо запустить тонкий или толстый клиент.'");
#КонецЕсли
...
#Если Не ТолстыйКлиентУправляемоеПриложение И Не ТолстыйКлиентОбычноеПриложение Тогда
	ВызватьИсключение НСтр("ru = 'Для сравнения необходимо запустить толстый клиент.'");
#Иначе
...
Функция Подключаемый_ПроверитьВхождениеРолейИВидовДоступаВПрофили()
	...
	ПредставлениеОшибки = НСтр("ru = 'Роль или вид доступа не задействована ни в одном из профилей'");
	ОписаниеИсправления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Это означает, что работа программы с этой ролью (видом доступа) не тестируется и не демонстрируется в демо-базе.
			|Для исправления: 
		...
		"_ДемоСтандартныеПодсистемы.ПриЗаполненииПоставляемыхПрофилейГруппДоступа");
...
Функция Подключаемый_НекорректныеСвязиМеждуПодсистемами()
	...
	ПредставлениеОшибки = НСтр("ru = 'Недопустимо наличие недокументированных связей между подсистемами'");
	ОписаниеИсправления = НСтр("ru = 'Связи между подсистемами делают невозможным выборочное встраивание библиотеки в конфигурацию, поэтому их следует избегать. 
...
Процедура ОпределитьОшибкиУсловныхВызовов(ФайлМодуля, Знач СтрокаТекста, Ошибки, СписокИспользуемыхПодсистем, СтрокаУсловногоВызова)
	...
	Если ОбъектыИсключенияПроверкиУсловныхВызовов = Неопределено Тогда
		ОбъектыИсключенияПроверкиУсловныхВызовов = Новый Соответствие;
		ОбъектыИсключенияПроверкиУсловныхВызовов["ОбщегоНазначенияКлиентСервер"] = Истина;
		ОбъектыИсключенияПроверкиУсловныхВызовов["ПроверкаБСППередСборкой"] = Истина;
		ОбъектыИсключенияПроверкиУсловныхВызовов["ЗависимостиПодсистем"] = Истина;
	КонецЕсли;
	Если ОбъектыИсключенияПроверкиУсловныхВызовов[ПолноеИмяСтруктура.ИмяОбъекта] <> Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если ПодсистемыИсключенияПроверкиУсловныхВызовов = Неопределено Тогда
		ПодсистемыИсключенияПроверкиУсловныхВызовов = Новый Массив;
		ПодсистемыИсключенияПроверкиУсловныхВызовов.Добавить("ИнтернетПоддержкаПользователей");
		ПодсистемыИсключенияПроверкиУсловныхВызовов.Добавить("ТехнологияСервиса");
		ПодсистемыИсключенияПроверкиУсловныхВызовов.Добавить("ИнтеграцияС1СДокументооборотом");
	КонецЕсли;
	...
	Пока Истина Цикл
		...
		ЭтоПодсистемаИсключение = Ложь;
		Для каждого ИмяПодсистемы Из ПодсистемыИсключенияПроверкиУсловныхВызовов Цикл
			Если СтрНайти(УсловныйВызов, ИмяПодсистемы) > 0 Тогда
				ЭтоПодсистемаИсключение = Истина;
				Продолжить;
			КонецЕсли;
		КонецЦикла;
		Если ЭтоПодсистемаИсключение Тогда
		...
		Если Не ИмяПодсистемыЗаданоКорректно(УсловныйВызов, СписокИспользуемыхПодсистем) Тогда
			ТекстСообщения = ?(ПустаяСтрока(ТекстСообщения),
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1: %2'"), 
					ПолноеИмяСтруктура.ПолноеИмяМодуля, УсловныйВызов),
				УсловныйВызов);
...
Функция ИмяПодсистемыЗаданоКорректно(УсловныйВызов, СписокИспользуемыхПодсистем)
	...
	Если СтрНайти(УсловныйВызов, "ЭлектронноеВзаимодействие") > 0
		Или СтрНайти(УсловныйВызов, "РегламентированнаяОтчетность") > 0
		Или СтрНайти(УсловныйВызов, "ПоддержкаОборудования") > 0 Тогда
		Возврат Истина;
	КонецЕсли;
...
// документации. Кроме того, появляется зависимость от конкретной роли, чего рекомендуется избегать, так как
